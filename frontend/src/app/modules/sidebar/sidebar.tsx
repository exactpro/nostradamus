...
class Sidebar extends React.Component<Props, SideBarState> {
	...
	checkSideBarClickTarget = (e: any) => {
		if (
			!(
				(this.settingsRef.current && this.settingsRef.current.contains(e.target)) ||
				(this.virtualAssistantRef.current && this.virtualAssistantRef.current.contains(e.target))
			)
		)
			this.closeSlidingWindow();
	};
		...
		return (
			...
			>
				<div className="navigation-bar__inner">
					<img
						className={cn("navigation-bar__logo", {
							"navigation-bar__logo_hidden": this.state.isOpen,
						})}
						src={logo}
						alt="Nostradamus Logo"
					/>
					<img
						className={cn("navigation-bar__logo-full", {
							"navigation-bar__logo-full_hidden": !this.state.isOpen,
						})}
						src={logoFull}
						alt="Nostradamus Logo"
					/>
					...
					<div className="navigation-bar__user">
						...
						<div className="navigation-bar__user-data">
							...
							<div className="navigation-bar__user-email">
               ...
               Account Management
						...
						<ul>
							<Link
								...
								className={cn("navigation-bar__menu-item", {
									"navigation-bar__menu-item_active": pathname === RouterNames.analysisAndTraining,
								})}
							>
								<div className="navigation-bar__menu-item-icon">
									<Icon type={IconType.analysis} size={IconSize.big} />
								</div>

								<div className="navigation-bar__menu-item-text">Analysis & Training</div>
							</Link>

							<Link
								to={RouterNames.descriptionAssessment}
								className={cn("navigation-bar__menu-item", {
									"navigation-bar__menu-item_active":
										pathname === RouterNames.descriptionAssessment,
								})}
							>
								<div className="navigation-bar__menu-item-icon">
									<Icon type={IconType.description} size={IconSize.big} />
								</div>

								<div className="navigation-bar__menu-item-text">Description Assessment</div>
							</Link>
							...
							>
								<div className="navigation-bar__menu-item-icon">
									<Icon type={IconType.QAMetrics} size={IconSize.big} />
								</div>

								<div className="navigation-bar__menu-item-text">QA Metrics</div>
							</Link>
						</ul>
					</nav>
					...
					<nav className="navigation-bar__menu navigation-bar__menu_position_bottom">
						<ul ref={this.virtualAssistantRef}>
							...
							>
								<div className="navigation-bar__menu-item-icon">
									<Icon type={IconType.chatBeta} size={IconSize.big} />
								</div>

								<div className="navigation-bar__menu-item-text">{SideBarTabs.virtualAssistant}</div>
							</li>
						</ul>

						<ul ref={this.settingsRef}>
							<Tooltip
								message="Uploading data. Please wait a few minutes."
							...
							>
								...
								>
									...
									<div className="navigation-bar__menu-item-text">{SideBarTabs.settings}</div>
								</li>
							</Tooltip>
						</ul>
					</nav>
