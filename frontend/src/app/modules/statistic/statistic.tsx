	...
	return (
		<table className="statistic-table">
			...
			<tr className="statistic-table__row">
				...
				{
					statisticColumnKeys.map((columnName, index)=><th key={index} className="statistic-table__column-header">{columnName.toUpperCase()}</th>)
				}
			</tr>
			</thead>
				...
				Object.keys(props.statistic).map((sectionName: string, sectionIndex: number)=>{
					...
					return (
						<tr className="statistic-table__row" key={sectionIndex}>
							...
							{
								statisticColumnKeys.map((columnName, index) => <td key={index} className="statistic-table__cell">{sectionObject[columnName]}</td>)
							}
						</tr>)
					})
			}
