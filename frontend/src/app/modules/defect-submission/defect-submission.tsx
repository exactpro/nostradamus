...
class DefectSubmission extends React.Component<Props> {
	...
	maxValue: number = 0;
	minValue: number = 0;
	segmentWidth: number = 120;
	yAxisBarWidth: number = 70;
	yAxisSteps: number = 7;
	scrollableGraphWidth: number = 0;
	...
	constructor(props: Props){
		...
		this.graphData = Object.entries(this.props.defectSubmission.created_line).map(([data, value]: [string, number]) => ({
			data,
			createdValue: value,
		...
		const permMaxValue = Math.max(this.graphData[this.graphData.length - 1].createdValue, this.graphData[this.graphData.length - 1].resolvedValue);
	...
	componentDidMount = () => {
		...
		const svgElem: any = this.graphRef.current?.getElementsByClassName("defect-submission__graph-svg");
		...
		{ 
			...
			if(wrapperWidth>this.segmentWidth*this.graphData.length) this.segmentWidth = 0.975 * wrapperWidth / ((this.graphData.length - 1) || 2);
			
			this.scrollableGraphWidth = (this.graphData.length - 1? (this.graphData.length - 1) * this.segmentWidth: this.segmentWidth * 2) + 10;
	...
	displayTooltip = (coords: number[], dataIndex: number, lineIndex: number) => {
		...
		this.graphTooltipRef.current.style.cssText = `left: ${coords[0] - 15}px; top: ${coords[1] - 5}px;`;
	}

	euclideanDistance = (x1: number, y1: number, x2: number, y2: number) => Math.hypot(x2-x1, y2-y1)

	getClosestPointToMouse = (x: number, y: number, coordsArr: LineCoordinateTemplate[]) => {
		let minDistanceArr: number[] = [], minDistance = Infinity, minDistanceLineIndex = 0, minDistanceLineEndIndex = 0;

		const distanceCalculator = ( pointX: number, pointY: number, lineIndex: number, endIndex: number) => {
	...
	mouseEnterDisplayTooltip = (e: any, coordsArr: LineCoordinateTemplate[], index: number) => { 
		...
		const minDistanceObject = this.getClosestPointToMouse( e.clientX - rect.x + this.segmentWidth*index, e.clientY - rect.y, coordsArr);
		this.displayPoint(minDistanceObject.minDistanceArr, minDistanceObject.minDistanceLineIndex);
		this.displayTooltip(minDistanceObject.minDistanceArr, index + minDistanceObject.minDistanceLineEndIndex, minDistanceObject.minDistanceLineIndex);
	}
	...
	buildGraphObject = (startLineY: number, endLineY: number, lineIndex: number, segmentIndex: number, objectType : 'line' | 'circle', coordinateArr: LineCoordinateTemplate[]) => {
		if(!this.potentialGraphHeight || segmentIndex===this.graphData.length || (objectType === 'line' && segmentIndex === 0)) return null;

		const endXCoordinate = objectType === 'line'? segmentIndex*this.segmentWidth + 5: this.scrollableGraphWidth/2;
		...
		if(objectType === 'line')
		return <path d={`M${coordinates.startLineX} ${coordinates.startLineY} L${coordinates.endLineX} ${coordinates.endLineY}`} 
   					 className="line-chart__graph-line"
					 stroke={this.colorSchema[lineIndex]}/>
	
		return <circle cx={coordinates.startLineX + 1.5} cy={coordinates.startLineY} r={2} fill={this.colorSchema[lineIndex]} className="line-chart__circle"/>
	...
	renderGraphLine = (index: number) => {
		...
		let endXCoordinate = (index)*this.segmentWidth + 5; 

		const textAnchor = index === 0? "start": index===this.graphData.length - 1? "end": "middle";

		const coordinateArr: LineCoordinateTemplate[] = [];
		const createdLine = this.buildGraphObject(this.graphData[index - 1]?.createdValue*pointHeight, this.graphData[index]?.createdValue*pointHeight, 0, index, 'line', coordinateArr);
		const resolvedLine = this.buildGraphObject(this.graphData[index - 1]?.resolvedValue*pointHeight, this.graphData[index]?.resolvedValue*pointHeight, 1, index, 'line',coordinateArr);
			...
			<React.Fragment key={index}>
				{
					index !== 0 && 
					<>
						<rect x={endXCoordinate - this.segmentWidth} width={this.segmentWidth} height={this.potentialGraphHeight} 
							  className="line-chart__rect"
							  onMouseMove={(e: any)=>this.mouseEnterDisplayTooltip(e, [...coordinateArr], index - 1)}/>
						{ createdLine }
						{ resolvedLine }
					</>
				}
				
				<text x={endXCoordinate} y={this.potentialGraphHeight} textAnchor={textAnchor} className="line-chart__axis-text">{this.graphData[index].data}</text>
				<line x1={endXCoordinate} x2={endXCoordinate}  y1={0} y2={this.potentialGraphHeight}
	...
	renderGraphPoint = () => {
		...
		const coordinateArr: LineCoordinateTemplate[] = [];
		const createdCircle = this.buildGraphObject(this.graphData[0]?.createdValue*pointHeight, this.graphData[0]?.createdValue*pointHeight, 0, 0, 'circle', coordinateArr);
		const resolvedCircle = this.buildGraphObject(this.graphData[0]?.resolvedValue*pointHeight, this.graphData[0]?.resolvedValue*pointHeight, 1, 0, 'circle',coordinateArr);															

		return (
			<>
				<rect x={3} width={this.scrollableGraphWidth} height={this.potentialGraphHeight}
					  onMouseMove={(e: any)=>this.mouseEnterDisplayTooltip(e, [...coordinateArr], 0)}
					  className ="line-chart__rect"/>
				
				<text x={endXCoordinate} y={this.potentialGraphHeight} textAnchor="middle" className="line-chart__axis-text">{this.graphData[0].data}</text>
	...
	renderYAxis = () => {
		...
		for(let i=0; i<=beautifulSteps; i++) 
			yAxis.push(<text key={i} 
				 			 x={this.yAxisBarWidth - 20} 
				 			 y={ (this.potentialGraphHeight - 10)*(1 - 1/beautifulSteps*i) - 10} 
							 textAnchor="end"
							 className="line-chart__axis-text">{beautifulTick*i}</text>)
			...
			<div className="defect-submission">
				...
				<div className="defect-submission-legend">
					<div className="defect-submission-legend__wrapper">
						<div className={cn("defect-submission-legend__point", "defect-submission-legend__point_created")}></div>
						<p className="defect-submission-legend__title">Created issues - {this.props.defectSubmission?.created_total_count}</p>
					</div>
					<div className="defect-submission-legend__wrapper">
						<div className={cn("defect-submission-legend__point", "defect-submission-legend__point_resolved")}></div>
						<p className="defect-submission-legend__title">Resolved issues - {this.props.defectSubmission?.resolved_total_count}</p>
					</div>
				</div>
				...
				<div className="defect-submission__scroll-container" ref={this.graphRef}>
					...
					<div className="defect-submission__graph">
						...
						<div>
							...
							<Tooltip message=""
									 duration={0} 
									 tooltipOuterRef={this.graphTooltipRef}>
								<div className={cn("defect-submission__graph-pointer", "defect-submission__graph-pointer_created")} ref={this.graphPointerRef}></div>
							</Tooltip>
							
							<svg width={this.scrollableGraphWidth} height="calc(100% - 25px)" className="defect-submission__graph-svg"> 
								{	
