...
class NumericRange extends React.Component<Props, State> {
	...
	handleChanges = (edge: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
		let current_value = this.field.current_value as FilterFieldNumberValue;
		let newValue: FilterFieldNumberValue = [current_value[0], current_value[1]];
	...
	preventIncorrectInputValue = (event: React.KeyboardEvent<HTMLInputElement>) => {
		let allowedValues: string = '0123456789';
	...
	render() {
		...
		return (
			<div className="numeric-range">
				...
				<span className="numeric-range__separator">
					—
				</span>
