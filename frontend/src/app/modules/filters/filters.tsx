...
export enum FiltersPopUp{
	noDataFound = "Data cannot be found. Please change filters.",
}
...
export class Filters extends React.Component<InnerProps> {
	...
	checIsFiltersDefault = (filters: FilterFieldBase[]) =>
		filters.reduce(
			(prevVal: boolean, item: FilterFieldBase) => prevVal && !item.current_value.length,
			true
		);

	shouldComponentUpdate = (nextProps: InnerProps) => {
		if (
			!this.checIsFiltersDefault(this.props.filters) &&
			this.checIsFiltersDefault(nextProps.filters)
		)
			this.resetAll();
						...
						{this.filters.fields
							...
							.map((field, index) => (
								<div className="filters__filter" key={index}>
									<div className="filters__filter-name">{field.name}</div>
