...
class Slider extends React.Component<SliderProps, SliderState> {
	...
	nextSlide = () => {
		...
		if (this.props.endless) {
			indexOfActiveSlide =
				this.state.indexOfActiveSlide < this.props.slides.length - 1
					? this.state.indexOfActiveSlide + 1
					: 0;
		} else {
			indexOfActiveSlide =
				this.state.indexOfActiveSlide < this.props.slides.length - 1
					? this.state.indexOfActiveSlide + 1
					: this.state.indexOfActiveSlide;
		}
	...
	prevSlide = () => {
		...
		if (this.props.endless) {
			indexOfActiveSlide =
				this.state.indexOfActiveSlide > 0
					? this.state.indexOfActiveSlide - 1
		...
		return (
			<div className={cn("slider", this.props.overlayManager ? "slider_type_overlay" : null)}>
				...
				<ul
					className={cn(
						"slider__manager",
						this.props.overlayManager ? "slider__manager_type_overlay" : null
					)}
				>
					<li
						className={cn(
							"slider__button slider__button_prev",
							this.state.indexOfActiveSlide === 0 && !this.props.endless
					...
					>
						<Icon
							type={IconType.left}
							size={this.props.overlayManager ? IconSize.big : IconSize.normal}
						/>
					</li>
					...
					{this.props.slides.map((slide, i) => (
						<li
							...
							className={cn(
								"slider__navigation",
								this.state.indexOfActiveSlide === i && "slider__navigation_active"
							)}
						>
							<span className="slider__point" />
						</li>
					))}
					...
					<li
						className={cn(
							"slider__button",
							this.state.indexOfActiveSlide === this.props.slides.length - 1 && !this.props.endless
								? "slider__button_disabled"
								: null
						)}
