...
class DonutChart extends React.Component<IProps, iState> {
	...
	shiftDonutSegment = (segmentId: string, target: SVGClipPathElement, dl = 2) => {
	...
	render() {
		const chartData = this.data.filter((item) => item.value !== 0);
			...
			<div className={cn("donut-chart", this.props.className)}>
				...
				<div className="donut-chart-legend">
					{this.data.map((item, index) => {
						const bgColor = this.props.colorSchema ? this.props.colorSchema[index] : "transparent";
						return (
							<div className="donut-chart-legend__wrapper" key={index}>
								<div
									className="donut-chart-legend__circle-pointer"
				...
				<div className="donut-chart-wrapper" ref={this.pieRef}>
					<p
						style={{ color: this.state.percentageData.color }}
