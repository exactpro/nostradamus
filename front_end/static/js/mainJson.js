...
function addField(parent_div, div_class, label_class,
    ...
    if(field_type_backend == 'Substring'){
        i_field.type = field_type
        i_field.className = field_class[0];
    }
    if(field_type_backend == 'Numeric'){
...
function field_factory(id){
    // get parent div
    var groups = ['special_attributes', 'mandatory_attributes', 'areas_of_testing']
    groups.forEach(function(group){
        var parent_div = document.getElementById(id);
        // create group div for fields
        if(group == 'areas_of_testing' && jsonDictionary['fields'].hasOwnProperty('areas_of_testing')){
            if (Object.keys(jsonDictionary['fields'][group]).length > 0){
        ...
        if(group == 'mandatory_attributes' && Object.keys(jsonDictionary['fields'][group]).length > 0){
        ...
        for (var key in jsonDictionary['fields'][group]){
            ...
            else if (group == 'areas_of_testing'){
                field_tag = 'select'
                field_type = 'Boolean'
            }
            else{
                field_tag = 'input'
                if(['String', 'Substring', 'Substring_array'].includes(jsonDictionary['fields'][group][key]['type'])){
                    field_type = 'String'
                    div_class = 'form-group form-group-custom'
                    }
                if(['Numeric', 'Date'].includes(jsonDictionary['fields'][group][key]['type'])){
                    field_type = 'Numeric'
                    ...
                    if(jsonDictionary['fields'][group][key]['type'] == 'Numeric')
                        field_class = ['form-control input-custom req-selectCom1', "form-control input-custom req-selectCom3"]
                        }
                if(['Boolean'].includes(jsonDictionary['fields'][group][key]['type'])){
                    field_tag = 'select'
                    }
                }
                if (group == 'areas_of_testing'){
                    addField(parent_div, div_class, label_class, inner_div_class,
                        jsonDictionary['fields'][group][key]['gui_name'],jsonDictionary['fields'][group][key]['gui_name'], field_class, field_tag, field_type,
                        'Boolean')
                }
                else{
                    addField(parent_div, div_class, label_class, inner_div_class,
...
$('#markup').change(function setAreas(){
    if($('#markup').val()=='yes'){
        $("#areasDiv").removeClass('disabledfilter');
        $("#areas").prop('required',true);
        sessionStorage['markup'] = $('#markup').val();
        }
    else {
...
function setMessage(){
    ...
    if('undefined'.localeCompare(jsonDictionary['plot']) != 0)
        if(jsonDictionary['plot']['defect_submission_chart'] == 'error'){
...
function disableButtons(jsonDictionary){
    if(jsonDictionary['message'].length == 0){
        $("#Apply").prop("disabled",true);
        $("#Build").prop("disabled",true);
        $("#Save").prop("disabled",true);
    }
...
function dynamic_bugs(data){
                var relFreq = data['defect_submission_chart'];
                var xLine = relFreq[0];
                var yLine = relFreq[1];
                var ctx = $("#dynamic");
                var myChart = new Chart(ctx, {
...
function choosePlot(jsonDictionary){
    ...
    for(key in plotJson){
        if(key == 'Relative Frequency'){
            distributionChart = relative_frequency(plotJson['Relative Frequency']);
        }
        if(key == 'defect_submission_chart'){
                    if(plotJson['defect_submission_chart'] == 'error'){
                        dynamicChart = defaultDynamicChart();
                    }
                    else{
                        dynamicChart = dynamic_bugs(plotJson['defect_submission_chart']);}
                  }
        }
...
$('#period').change(function ajaxDynamic() {
    $.ajax({
            type: "POST",
            url: get_periodAJAX('/analysis_and_training/defect_submission_chart'),
            beforeSend: setLoad(),
                ...
                if (response.redirect) {
                    ...
                    if (ct.indexOf('html') > -1)
                        document.write(response);
                         ...
                         setMessage();
                         dynamicChart = updateDynamicPlot(dynamicChart, jsonDictionary['plot']['defect_submission_chart']);
                         hideLoad();
                         setFileName();
                         }
                     }
...
function ajaxXDistribution() {
    $.ajax({
            type: "POST",
            url: '/analysis_and_training/build_chart/distribution/',
...
function updateDistributionPlot(distributionChart, jsonDictionary){
    ...
    for(key in jsonDictionary['plot']){
        if(key == 'Relative Frequency'){
            distributionChart.data.datasets[0].labels = jsonDictionary['plot']['Relative Frequency']['coordinates'][0];
            distributionChart.data.datasets[0].data = plot(jsonDictionary['plot']['Relative Frequency']['coordinates'][0],jsonDictionary['plot']['Relative Frequency']['coordinates'][1]);
            ...
            distributionChart.options.scales.yAxes[0].ticks.stepSize = setStepSizeY(jsonDictionary['plot']['Relative Frequency']['coordinates'][1], 0);
            distributionChart.options.scales.xAxes[0].ticks.max = findMax(jsonDictionary['plot']['Relative Frequency']['coordinates'][0], 0, jsonDictionary['plot']['Relative Frequency']['scale']);
            distributionChart.options.scales.xAxes[0].ticks.stepSize = setStepSize(jsonDictionary['plot']['Relative Frequency']['stepSize']);
        ...
        if(key == 'Frequency density'){
            if(jsonDictionary['plot']['Frequency density']['coordinates']['line'][0] == -1 && jsonDictionary['plot']['Frequency density']['coordinates']['line'][1] == -1){
                distributionChart.data.datasets[0].labels = jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][0];
                distributionChart.data.datasets[0].data = plot(jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][0], jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][1]);
                distributionChart.data.datasets[0].steppedLine = true;
                distributionChart.data.datasets[0].borderColor = ["rgb(54, 162, 235)"];
                distributionChart.data.datasets[0].backgroundColor = ["rgba(54, 162, 235, 0.3)"];
                distributionChart.data.datasets[0].borderWidth = chooseThickness('relative');
                distributionChart.data.datasets[1] = {};
                distributionChart.options.scales.yAxes[0].ticks.stepSize = setStepSizeY(jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][1], 0);
                distributionChart.options.scales.xAxes[0].ticks.max = findMax(jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][0], 0, '');
                distributionChart.options.scales.xAxes[0].ticks.stepSize = setStepSize('');
                distributionChart.options.elements.point.radius = 0.5;
                distributionChart.update();
                return distributionChart;
                }
            distributionChart.data.datasets[0].labels = jsonDictionary['plot']['Frequency density']['coordinates']['line'][0];
            distributionChart.data.datasets[0].data = plot(jsonDictionary['plot']['Frequency density']['coordinates']['line'][0],jsonDictionary['plot']['Frequency density']['coordinates']['line'][1]);
            ...
            distributionChart.data.datasets[1] = {};
            distributionChart.data.datasets[1].data = plot(jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][0],jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][1]);
            distributionChart.data.datasets[1].labels = jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][0];
            ...
            distributionChart.options.scales.yAxes[0].ticks.stepSize = setStepSizeY(jsonDictionary['plot']['Frequency density']['coordinates']['line'][1], jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][1]);
            distributionChart.options.scales.xAxes[0].ticks.max = findMax(jsonDictionary['plot']['Frequency density']['coordinates']['line'][0], jsonDictionary['plot']['Frequency density']['coordinates']['histogram'][0], jsonDictionary['plot']['Frequency density']['scale']);
            distributionChart.options.scales.xAxes[0].ticks.stepSize = setStepSize(jsonDictionary['plot']['Frequency density']['stepSize']);
            distributionChart.options.elements.point.radius = 0.5;
            distributionChart.update();
...
function get_period(action){
    ...
    if(action == '/analysis_and_training/filtrate')
        $('#filterForm').attr('action',action+'?ReferringTo='+$('#ReferringTo').val());
    if(action == '/analysis_and_training/reset_filter')
        $('#resetForm').attr('action',action+'?period='+$('#period').val());
    if(action == '/download_subset')
...
function get_periodAJAX(action){
    ...
    if(action == '/analysis_and_training/defect_submission_chart')
        return action+'?period='+$('#period').val();
    if(action == '/analysis_and_training/filtrate')
        return action+'?period='+$('#period').val();
    if(action == '/analysis_and_training/reset_filter')
...
function statInfo(jsonDictionary){
    ...
    var commentStat = statInfo['comments_stat']
    ...
    var attachmentStat = statInfo['attachments_stat']
...
if('undefined'.localeCompare(jsonDictionary['statistical_info']) != 0)
        statInfo(jsonDictionary['statistical_info']);
...
function setTopFreq(){
    ...
    jsonDictionary['attributes']['frequently_terms'] = jsonDictionary['attributes']['frequently_terms'].map(function(field){return " "+field})
    $("#frequently_terms").html((jsonDictionary['attributes']['frequently_terms'].slice(0, parseInt(sessionStorage['moreFreq'], 10)+20).join()));
...
$('#ReferringTo').change(function significanceTop() {
                         $.ajax({
                                 type: "POST",
                                 url: '/analysis_and_training/significant_terms',
                                 ...
                                 success: function(response, status, xhr){
                                     if (response.redirect) {
                                         ...
                                         if (ct.indexOf('html') > -1)
                                           ...
                                           if(response['significant_terms'] == 'error'){
                                               ...
                                               // for correct hyphenation add space for each world
                                               if (response['significant_terms'] != null){
                                                    response['significant_terms'] = response['significant_terms'].map(function(field){return " "+field})
                                                    $("#significanceTop").html(response['significant_terms'].join());
                                               }
                                               else{
                                                    $("#significanceTop").html(response['significant_terms']);
                                               }
                                               hideLoad();
                                               setFileName();
                                           }
...
function filtering() {
    ...
    $.ajax({
         type: "POST",
         url: '/analysis_and_training/filtrate?ReferringTo='+$('#ReferringTo').val(),
         beforeSend: setLoad(),
             ...
             if (response.redirect) {
                 ...
                 if (ct.indexOf('html') > -1)
                    document.write(response);
                       ...
                       jsonDictionary = response;
                       if (jsonDictionary['attributes']['frequently_terms'] == 'error' || jsonDictionary['attributes']['significant_terms'] == 'error' || jsonDictionary['frequently_terms'] == 'error'){
                           document.getElementById('message').innerHTML = 'not find regularExpression.csv';
                           $('html, body').animate({scrollTop: 0}, 600);
                           ...
                           hideLoad();
                            ...
                            attributesAJAX(jsonDictionary['attributes'], Object.keys(jsonDictionary['categoric']));
                            dynamicChart = updateDynamicPlot(dynamicChart, jsonDictionary['plot']['defect_submission_chart']['defect_submission_chart']);
                            distributionChart = updateDistributionPlot(distributionChart, jsonDictionary);
                            $('html, body').animate({scrollTop: 0}, 600);
                            $("#Reset").prop("disabled",false);
                            setFileName();
...
function reset1() {
    $.ajax({
            type: "POST",
            url: '/analysis_and_training/reset_filter',
...
function setLoad() {
    ...
    groups.forEach(function(group){
        for(var el in jsonDictionary['fields'][group])
                if(['String', 'Substring_array', 'Substring', 'Date'].includes(jsonDictionary['fields'][group][el]['type'])){
                    if(jsonDictionary['fields'][group][el]['type'] == 'Date'){
...
function checkComAtTt(jsonDictionary){
    ...
    groups.forEach(function(group){
        for(var el in jsonDictionary['fields'][group])
                if(jsonDictionary['fields'][group][el]['type'] == 'Numeric'){
                    $("[id='"+el+0+"']").val($("[id='"+el+0+"']").val().trim())
                    $("[id='"+el+1+"']").val($("[id='"+el+1+"']").val().trim())
...
function training_model(){
    $.ajax({
        type: "POST",
        url: '/analysis_and_training/train',
        beforeSend: setLoad(),
        success: function(response, status, xhr){
            if (response.redirect) {
                 window.location.href = response.redirect;
...
function check_size_files(){
    ...
    */
    ...
    groups.forEach(function(group){
        for(var el in jsonDictionary['fields'][group]){
            if(jsonDictionary['fields'][group][el]['type'] == 'Date' || jsonDictionary['fields'][group][el]['type'] == 'Numeric'){
                if (jsonDictionary['fields'][group][el]['type'] == 'Date'){
